import * as listenerUtils from "../../core/listeners";
import { runCallback } from "../../core/utils";
import { 
    fromInspectable,
    isProxy 
} from "./globals";
    ...
    return {
        apply: async function (target, thisArg, argumentsList) {
            try {
                ...
                let listeners = inspectable.listeners.functions?.[id]
                // Notify of Function Execution
                const pathStr = inspectable.path.slice(1).join(inspectable.options.keySeparator)
        ...
        set(target, prop, newVal, receiver) {   
            ...
            if (createListener) {
                if (typeof inspectable.options.globalCallback === 'function') {
                    ...
                    listenerUtils.set('setters', [...inspectable.path, prop], newVal, inspectable.options.globalCallback, {[id]: inspectable.root}, inspectable.listeners, inspectable.options)
                }
            }
